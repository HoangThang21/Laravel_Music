...
<div class="bodyhome">
                    ...
                    <div class="media">
                        ...
                        <div class="media-right">
                            ...
                            <div class="time-curent-media"data-song="{{ $nhactop10->id }}">
                                <span>{{ $duration }}</span><i class="loadmusic-dot bi bi-caret-right-fill"></i>
                            </div>
                            @if (Auth::guard('web')->check())
                                @php
                ...
                @foreach ($Nghesitop20 as $nstop20)
                    <div class="body-Contener_ns">
                        @foreach ($user as $item)
                            ...
                            @endif
                        @endforeach
                        <div class="name-ns" onclick="ClickNghesi({{ $nstop20->id }})">{{ $nstop20->tennghesi }}
                        </div>
                        @php
    ...
    <div class="song-search-click" style="display:none">
        <div class="table-music">
            ...
            <div class="menu-media">
                @foreach ($albumsearch as $albsear)
                    @foreach ($nhacsearch as $nhactop10)
                        @if ($albsear->id == $nhactop10->album_idnhac)
                                        ...
                                        <div class="menu-right-media ">
                                            ...
                                            <div class="nhaccho" data-cho="{{ $nhactop10->maNhac }}"
                                                data-gia="{{ $nhactop10->gia }}"><i
                                                    class="bi bi-phone-vibrate"></i>Cài
                                                nhạc
                                                chờ
                                            </div>
                                            <div class="sendchat" data-sendchat="{{ $nhactop10->id }}"><i
    ...
    <div class="album-search-click"style="display:none">
        <div class="contener">
            ...
            <div class="list-menu-contenter-search">
                @foreach ($albumsearch as $albsear)
                    @foreach ($nghesi as $item)
                        @if ($item->id == $albsear->nghesi_idalbum)
                            <div class="body-Contener">
                                <div class="item-body-content" data-album="{{ $albsear->id }}">
                                    <img src="../../images/{{ $albsear->hinhalbum }}" alt="">
                                    <div class="hover-item-body-content">
                                        <i class="bi bi-caret-right-fill"></i>
                                    </div>
                                </div>
                                <div class="title-body-content">{{ $albsear->tenalbum }}</div>
                            </div>
                        @endif
                    @endforeach
                @endforeach
                        ...
                        @if ($item->id == $albsear->nghesi_idalbum)
                            <div class="body-Contener-search ">
                                @foreach ($user as $item)
                                    ...
                                    @endif
                                @endforeach
                                <div class="name-ns" onclick="ClickNghesi({{ $nstop20->id }})">
                                    {{ $nstop20->tennghesi }}
                                </div>
                                @php
                                    $inputString = $nstop20->quantam;
                            ...
                            @else
                                ...
                                @else
                                    @if (Auth::guard('google')->check())
                                        @php
                                            $isInterested =
                                                strpos($nstop20->quantam, $ttnguoidung->id . 'gg') !== false;
                                        @endphp
                                        <div class="quantam"
                                            data-quantam="{{ $nstop20->id }}"@if ($isInterested) {{ `style="background: blue; color:#fff"` }}
                                @else
